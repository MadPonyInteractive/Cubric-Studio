...
- **Status (2026-09-15T07:25Z):** checklist 3/3, PLAN COMPLETE. Phase 3 verified by Fabio ("1") and the carried typedef text is done (MPI-751 released `types.js`, reply on `63274eb7`). **UNCOMMITTED:** `MpiModelManager.js`/`.css`, `docs/model-library.md`, `docs/releases/UNRELEASED.md`, `js/components/types.js` (3 hunks, MpiModelManagerProps + MpiFlowLibraryProps only). Claim `1aa63caa…` -> `complete`. MPI-751 fix 8 (LandingPages -> Organisms) waits for this to be COMMITTED. Phase 2 commit `2d13cf47` still unpushed (master CI red on a peer spec). **Next action:** `mpi-end-session` — commit by pathspec, rule-map drift question (Preservation Notes), close the card.
...
- [x] **Phase 2 Flow Library header** (2026-09-15, verified by Fabio) and **Phase 3 Model Library onto MpiFilterBar** (2026-09-15, verified by Fabio; typedef text included). Evidence in [validation.md](validation.md).
...
- 2026-09-14 (Phase 2): the factory's `instance.on()` returns nothing, so there is no `change` unsubscribe to store in `_unsubs`; teardown is `filterBar.el.destroy()` in `el.destroy`, same as the sheets.
- 2026-09-14 (Phase 2): side-by-side screenshots showed the bar's tags mixed-case while the Model Library's are UPPERCASE — the MM tags inherited `text-transform: uppercase` (+ nowrap, user-select) from `.mpi-btn`, and the foundations port dropped them. Fixed in `MpiFilterBar.css`; without it Phase 3 would have regressed the Model Library too.
- 2026-09-14 (Phase 2): `js/components/types.js` typedef edit blocked by MPI-749's live claim — left pending, message `da894aac…` sent. `04-overlay-and-shell.md` was already 202 lines at HEAD (budget 200); Phase 2 adds 8.
- 2026-09-14 (Phase 2): the plan's `app:isolated` screenshot step was done instead through the desktop-spec launcher (own port + profile, `:3000` untouched) with a temp spec, deleted after; the guard spec also asserts the cached `<img>` identity across a rebuild mechanically.
- 2026-09-15 (Phase 3): `docs/model-library.md` never described the filter row, so a short "Header row" section was added instead of an edit (file was already 266 lines, over the 200 budget). The MpiModelManagerProps typedef ("Media/Size/search filters") is also slightly stale and sits in the same claimed `types.js` — fold it into the typedef item.
- 2026-09-15 (Phase 3): `test-results/desktop` was held by a peer run (`ENOTEMPTY` before any test ran) — run Playwright with `--output=<scratchpad>` so the specs never share an output dir with a peer. Tier variants share tile names ("LTX 2.3", "Boogu Image Edit"), so a name-only set intersection miscounts; key tiles as name|media and compare multisets.
