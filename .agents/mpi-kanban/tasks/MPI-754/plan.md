...
- **Status (2026-09-15 06:55Z):** checklist 2/3. Phase 2 DONE — verified by Fabio in the app ("1"), **UNCOMMITTED**: `MpiFlowLibrary.js`/`.css`, `MpiFilterBar.css` (uppercase fix), `docs/playbooks/add-flow/04-overlay-and-shell.md`, new `tests/desktop/flow-library-filters.spec.js`. Claim `cf2e928f…` → `complete`. Letter-spacing question went unanswered → kept 0.14em. **Carried into Phase 3:** the MpiFlowLibrary typedef text in `js/components/types.js` — MPI-749 released it untouched, then MPI-751 took a live claim (`37193e01`, 06:48Z) for its tier relabels; message to MPI-751 sent (the MPI-749 one superseded). Phase 2 committed at handoff `2e12e953…` (types.js NOT included — not ours). **Next action:** Phase 3: re-check its gate AND the types.js claim (MPI-751 `37193e01`, message `63274eb7…`), take a fresh claim.
...
- [ ] **Flow Library typedef (carried from Phase 2).** In `js/components/types.js` MpiFlowLibraryProps (~1082-1092): drop "dev-gated" and "filters,", add a line naming the head (installed count + MpiFilterBar Media/Type/search). Only when no live peer claim covers types.js — check message to MPI-751 first; it may have folded it in. **Verify:** `grep -n -i "dev-gated\|filters, or re-sync" js/components/types.js` returns nothing for MpiFlowLibrary; CR count 0.
