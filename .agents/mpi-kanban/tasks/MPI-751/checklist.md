...
- [x] Fix 8 (flat, Fabio 2026-09-15; after MPI-754 Phase 3 committed e9063a7a): MpiFlowLibrary, MpiModelManager, MpiRunpodSettings, MpiLlmSettings -> `Organisms/<Name>/`; MpiRemote -> `Blocks/MpiRemote/`; MpiSettings, MpiAbout, MpiOllamaSetup, mpi-hotkeys stay. Lint 0, npm test 1036/1036, desktop library/remote specs 8/8. Parked rule: 6 -> 0 hits. `.claude/rules/component-events-primitives.md:263` old path waits on Fabio's go (rule file)
- [x] Land rule + test: `.eslint-rules/no-same-tier-component-import.js` + `tests/eslint-tier-rule.test.cjs` (16/16); `npm run lint` 0 on the new rule
