...
**Phase 4 state (2026-09-26, session 71db950c).** MPI-873 moved to `doing` (own `files.json`,
`checklist.md`). **Decided (Fabio): a named project may be CLOSED.** No new write path is needed:
`generationService.js:554` freezes `config._originProject`, and a closed origin gets its card via
`_addGroupsToClosedProject` -> `POST /project-groups` -> `updateProjectJson()`. Renderer design:
`folderPath` absent or equal to the open project (compare with slashes normalised and case folded,
then use `state.currentProject` ITSELF, never a second copy, or an open project gets written as a
closed one and loses the card on the next save) -> today's path; otherwise load it read-only
over `POST /get-project` and pass it as `_originProject`. Check the `maskGroupId` / `follow` /
extend lookups against a raw (unhydrated, history = id strings) project before allowing them.
MCP: `openFolder(t, folderPath)` validates `<folder>/project.json` (else `PROJECT_NOT_FOUND`),
`generate` + `list_cards` take `folderPath`, media staged into it, INSTRUCTIONS say name the
project on every generate. MPI-817 released the files at `2d925816a`. **CODE + TESTS + DOCS DONE, LIVE CHECK PASSED 2026-09-26 (MPI-873
`validation.md`); only commit + push remain** (MPI-873 `checklist.md` has the file list). The route
matches `folderPath` against `/list-projects` and dispatches the LIST's spelling, because
`_originIsOpen` and `activeGenerations.listFor` compare by exact string. Closed-target
`cardName` goes through `nameCard` -> `/project-groups` (upsert). Pinned settings still come
from the open panel. `agent-generation-relay.test.cjs` is still in MPI-817's claim, so the
route test lives in `tests/agent-target-project.test.cjs`. Live check: `app:isolated` on the
real engine (it shares Fabio's ComfyUI on 48188, so ask first). Open A, submit naming B,
switch to C mid-render; B gains 1 card on disk, A and C gain none; a plain submit lands in
the open one.

**Plan drift (2026-09-26):** the GIF tools are OUT of phase 4. They run in the renderer on items
of the OPEN project (`routes/connectorGif.js` over the job channel), so a closed-project GIF needs
its own write path; `list_cards` takes `folderPath` instead (the route already does).
