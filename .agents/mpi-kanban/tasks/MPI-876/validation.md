...
- **Cloud batch sidecar cost - CHECKED 2026-09-24 against Fabio's two FLUX Schnell batches of four** (`Deepinfra model tests`). All four sidecars of each batch carry `cost.usd: 0.002` (= 4 x $0.0005, the ONE call's bill) and the same `at` to the millisecond (`21:38:14.410Z`, `21:38:54.262Z`). The call's cost is right. The whole-call-on-every-card shape is the "native provider batch" row of MPI-855's brief, and it is now MEASURED for an agent batch: the ledger must sum per `at`, never per card. Nothing in the app sums per card today (grep `cost.usd` outside tests/docs: no readers).
- **Fabio, 2026-09-24: each card carries its OWN price.** The spend card keeps the batch total ($0.002 for four); each sidecar gets its share ($0.0005). Fix in `js/services/cloudExecutor.js`: `cost.usd / viewUrls.length` before `onComplete` (shared `at` kept, so the call is still identifiable). `tests/cloud-executor.test.cjs` "a batch of four…" now asserts `info.cost.usd === 0.0005`: 22/22 pass, and RED on HEAD's `cloudExecutor.js` (swapped in and back). A per-card sum is now right for a batch AND a fan-out; MPI-855's brief table ("each carrying the WHOLE call's cost") is stale for the batch row. **Fabio live, 2026-09-24 12:12 — PASS** (project shown as "Agent tests", folder `Anime Kids and Dog`): agent asked four FLUX Schnell images → ONE card `Run this 4 times on FLUX Schnell (Cloud)?` / `about $0.002 for 4 generations`, Yes → four Generating cards up front, t2i_009-012 landed 1344x768. All four sidecars: `usd 0.000492125`, one shared `at 12:12:52.518Z` — sum $0.00197, the call's bill. MPI-855 brief table updated to match.
- **Fabio, 2026-09-24:** `/connector/generate` does NOT refuse unconsented paid runs (CLI agent's user is its own gate). The price re-stated after Yes goes to the in-app agent session, not this card.
- **Fabio, 2026-09-24: no placeholders for fan-out.** A sequential run shows one card at a time; N cards up front belong to a real batch only. The pending option is dropped.
