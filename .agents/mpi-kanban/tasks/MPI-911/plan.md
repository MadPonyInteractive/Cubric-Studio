...
2. **The vendor notation replaces ours in both modes** (skill Phase 5 step 5): its section order,
   ...
   output is settled by the vendor's own worked examples, not by taste.
...
- [ ] **Research and inventory.** Re-run Phase 0: the top two community skills by stars
...
- [ ] **Recipe edit.** Every `contradicts` and `new` row becomes an edit or a written reason, in
...
- [ ] **Stage 1 sweeps.** Each touched mode green twice on the final text:
...
- [ ] **Guide folder.** Rewrite `docs/agent/models/seedance-2.0.md` as the router (pick it when,
...
- [ ] **Verbatim check, then hand back.** A scratchpad script (never committed) compares every
...
- Research and inventory, recipe edit, Stage 1 sweeps, guide folder, verbatim check.
