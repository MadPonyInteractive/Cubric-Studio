#!/usr/bin/env sh
# Cubric Studio - online updater (Linux).
...
# bundle ships this very file, and apply-update.cjs writes it with
# copyFileSync — an in-place truncate + rewrite of the same inode, while `sh`
# is still reading this script by byte offset. Without the wrapper the shell
# resumes at an offset into the NEW file and executes garbage (or nothing), so
# the relaunch below never runs. A function body is parsed in full before its
# first line executes, which is what makes the rewrite survivable.
set -u
