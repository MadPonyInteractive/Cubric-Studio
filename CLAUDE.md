...
| Debugging a live app bug | `docs/DEVELOPMENT.md` § Reading `logs/app.log` — filter by `[category]`, never read it whole. **Every Electron run, dev included, logs to `%APPDATA%\Cubric Studio\logs\app.log`** (`Cubric Vision` before MPI-708, 2026-09-17 — released v1.5.0 still writes there); the repo's `logs/` holds only test harnesses |
| **Master is red, a push is blocked, or Fabio is angry about CI again** | `docs/red-master.md` — five commands, four known causes. **Never the Kanban plugin.** Download the run's `test-failed-1.png` before theorising; green-locally-red-in-CI is the runner's bare disk, not a mystery |
