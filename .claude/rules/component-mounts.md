...
- `MpiToolOptionsCrop`   props: `{ viewer, kind: 'image'|'video' }`   — family `MpiRadioGroup` (RATIO/FREE/RESOLUTION; RESOLUTION is image-only) + orientation `MpiRadioGroup` (icon-only, RATIO only) + ratio `MpiRadioGroup` (icon-only, RATIO only, backed by `CROP_RATIOS` pure-aspect table incl cinema 2:1/1.85:1/21:9/2.39:1) + Width/Height `MpiInput` pair (RESOLUTION only) + Fill Outside `MpiColorPicker` (image only) + "Divisible by" `MpiInput` (default 16, hidden for RESOLUTION, above Apply) + apply (image) / snapshot+save (video) buttons. Persists `{ family:'ratio'|'free'|'resolution', orientation, label, divisible_by, res_w, res_h, fill_color }` under `project.toolSettings.crop` via `settings:tool:update` (toolKey `'crop'`). Pushes the box shape to the viewer: `viewer.el.setCropSize(res_w, res_h)` for RESOLUTION, else `viewer.el.setCropRatio(ratio|null)` — `null` = FREE (no aspect lock). Emits `apply { kind: 'image'|'video-save'|'video-snapshot' }`; `image` and `video-save` also carry `settings: { family, res_w, res_h, divisible_by, fill_color }` — the panel's LIVE values; the apply path uses them and never re-reads `toolSettings.crop`, which trails the panel by two debounces (MPI-795). The block passes them to `viewer.el.runCrop(settings)` (image, `MpiCanvasViewer._runCrop`) and `_handleCropSaveVideo(settings)` (video). Output W/H is rounded to `divisible_by` via `roundToDivisible` (js/utils/cropRounding.js) except for RESOLUTION, which resamples to exactly `res_w`×`res_h`; video sends `absoluteCropPx`, and `routes/videoCrop.js` uses it directly and skips even-snap. Crop drag honors Shift modifier (scales from rect center) via `Hotkeys.register('shift', …)` inside `CropManager`/`cropTool`.
...
- `MpiToolOptionsResize` props: `{ viewer, kind: 'image'|'video', currentItem? }` — Resolution Type `MpiRadioGroup` (SDXL/FLUX/FREE/MP/SCALE): SDXL/FLUX = orientation + ratio + x1/x2 multiplier; FREE = editable width/height `MpiInput`; MP = Megapixels `MpiInput` (1 MP = 1024×1024) and SCALE = ÷1.5/÷2/÷3/÷4 `MpiRadioGroup`, both deriving width/height from the source size via `deriveResizeDims` (js/utils/ratios.js), shown in a read-only width/height pair and recomputed per item (MPI-796). Then method/proportion/crop-position `MpiDropdown`, `MpiColorPicker` for pad color, divisible-by `MpiInput`, flip/rotation `MpiRadioGroup`, inline preview `<img>` slot, Apply `MpiButton`. Live preview runs the **image** `resize` workflow on a 512px-longest-edge thumbnail extracted from `viewer.el.getSourceElement()` (HTMLImageElement or HTMLVideoElement), with `width`/`height`/`divisible_by` proportionally scaled to thumb space. Result paints into the inline preview slot — viewer is never touched. Apply appends a new full-resolution entry via `enqueueGeneration` (`resize` for image, `resizeVideo` for video); preserves the source. Persists controls under `project.toolSettings.resize` via `settings:tool:update`.
...
**Instance API (on `el`):** `loadFrames(frames, {loop})`, `setFrames(frames, order?)`, `setFrameIndex(idx)`, `stepFrame(delta)`, `play()`, `pause()`, `enterMode('mask'|'crop'|'none')`, `exitMode()`, `setGifUrl(url)`, `setPreview(bool)`, `setGenerating(bool)`. Mask surface: `setMaskTint(url)`, `setCutoutPreview(url|null, proposalUrl|null)` (**display only**; the proposal is drawn green OVER the mask — `getFrameMaskURL`/`getCutMasks` read the `_masks` store, never the preview), `setTrackMasks(urls)`, `setTrackMask(idx, url)`, `clearFrameMasks('all'|idx)`, `getCutMasks()`, `getFrameMaskURL(idx)`, `setMaskBrushMode(mode)`, `setMaskBrushPreset(id)`, `setMaskInverted(bool)`, `setMaskBwView(bool)`, `setMaskPaintEnabled(bool)`, `setMaskOpacity(v)`, `clearMask()`. **No display flip (MPI-859):** the store holds WHAT GETS CUT and is drawn as itself — the image workspace's model; `getCutMasks()` flips once to the route's keep-space. Proposals: `setCandidateMasks(entries)`, `commitCandidates('add'|'subtract')`, `discardCandidates()`, `hasCandidates()`, `candidateAt(idx)` — a run proposes (green), a verb commits (white), and a proposal never reaches the cut. Crop surface: `setCropRatio(r)`, `setCropSize(w, h)`, `getCropRect()`. Query: `getFrameSize()`, `isToolOwningDrag()`. `destroy()`.
...
- Landing mascot crew (`initHeroCrew()`, `js/shell/heroCrew.js`, MPI-766). **NOT a component:** five hand-built mascot members in the hero. It follows `currentPage`, not a mount (the landing is only toggled `.hide`): built when the page becomes landing, `destroy()`ed when it leaves (ResizeObserver, ambient interval, per-member timeouts). Mounts `--held` (paused) and plays its entrance once `state.screenClear` is true. See `docs/shell.md` § heroCrew.js.
- Project-list foot: `MpiButton` `+ New project` in `#newProjectBtn`, and `MpiButton` `Open folder` in `#openFolderBtn` (Electron only, gated on `window.require`; invokes `choose-folder`). Both moved out of the hero in MPI-766.
...
- `MpiAgentChat` (standalone landing chat, MPI-774)   props: `{ standalone: true }`   slot: `#landingAgentSlot` — mounted once inside `initProjectUI()` (runs once at boot; no `el.destroy()` — "runs once — el.destroy() is never needed here"). Renders its OWN mascot + transcript + input row (textarea + send + drag-and-drop image attachments); does not go through the `agent:send` bus — `standalone: true` calls `agentSendMessage()` straight from its own internal `_sendMessage()`.
...
> `MpiLlmSettings` rebuilds on every `onOpen`, and its job rows again on every connection-models refresh (`_refreshModels` → `_paintRows`, each destroying the instance it replaces): `MpiDropdown` (`extraClasses:'mpi-dropdown--stacked'`) at `#mpiSettingsLlmEnhanceBackendSlot`, `#mpiSettingsLlmEnhanceModelSlot` (its group hides for ComfyUI), `#mpiSettingsLlmDescribeBackendSlot` and `#mpiSettingsLlmDescribeModelSlot` (its group shows only for Remote). There is no DeepInfra key field (MPI-737). Only while Ollama is the enhancement backend, `MpiOllamaSetup` mounts at `#mpiSettingsLlmOllamaSlot` with props `{ modelId }` (the pinned Ollama model, else `''`) and is destroyed when the backend changes; its component-local `state` event rebuilds the model dropdown only when a model's downloaded state CHANGES (it reports every second during a download, and a rebuild closes the dropdown under the pointer).
> **Shared LLM connection + Agent row (MPI-774).** Above the per-job rows, ONE connection block, rebuilt whenever the provider pick changes (`_renderConnDetails`), not only on `onOpen`: `MpiDropdown` (provider) at `#mpiSettingsConnProfileSlot` (`deepinfra`/`openrouter`/`openai`/`ollama`/`custom`, persisted via `Storage.setLlmConnection({ profileId })`), `MpiInput`+`MpiButton` (Base URL, `custom` only) at `#mpiSettingsConnUrlSlot`/`#mpiSettingsConnUrlSaveSlot`, `MpiInput` (password) + `MpiButton`×2 (API key Save/Clear, write-only via `secretsClient`, no state key; NOT mounted for the keyless `ollama` preset, whose `#mpiSettingsConnKeyGroup` is hidden) at `#mpiSettingsConnKeySlot`/`#mpiSettingsConnKeySaveSlot`/`#mpiSettingsConnKeyClearSlot`, and `MpiButton` (Test connection → `POST /llm/connection/probe`) at `#mpiSettingsConnProbeSlot`. The Agent row reads that ONE connection: no component for the backend — a text label (`Remote · <provider>`) written into `#mpiSettingsAgentBackend` (`.mpi-settings__fixed-value`), then `MpiDropdown` (model, from `GET /llm/connection/models?profileId=`, recommended-for-agent entries first) at `#mpiSettingsAgentModelSlot` persisted via `Storage.setAgentPrefs({ ...prev, model })`, `MpiDropdown` (mode: Auto / Ask first) at `#mpiSettingsAgentModeSlot` persisted via `Storage.setAgentPrefs({ ...prev, mode })`, and `MpiButton` (Test tool use → `POST /agent/probe`) at `#mpiSettingsAgentProbeSlot`. None of the connection or Agent-row controls touch `state.js` — `Storage` (localStorage) + `secretsClient` (main-process key store) only.
...
- `MpiAgentChat` (the shell agent-chat panel, MPI-774)   props: `{ standalone: false }`   slot: `#agent-panel-mount` (declared in `index.html`) — mounted once by `initAgentPanel()` (`js/shell/agentPanel.js`), called from `shell.js` right after `initAgentDispatch()`. App-lifetime, never destroyed. `initAgentPanel()` also opens the shared SSE bridge first (`agentInitStream()`, `js/services/agentService.js` — ONE `/agent/stream` EventSource for the renderer, re-emitting each named `agent:*` event on `Events`), so history replay sees live SSE events. Panel visibility toggles the `agent-panel-mount--open` class on the mount el, driven by `Events.onState('agentMode')` (mirrors `state.agentMode`, written by the MpiPromptBox Agent/Prompt toggle). **MPI-797:** `#agent-panel-mount` is a DIRECT child of `.main-area` (not of `#workspace-content`), absolute from under the topbar to the status bar; its width is `--agent-panel-w` on `.main-area`, and `workspace.css` gives `#workspace-content`, `#prompt-box-mount` and `#controls-mount` the same `margin-left` through `:has(> .agent-panel-mount--open)`.
- `MpiResizeHandle` (Primitive, the agent panel's right edge, MPI-797)   props: `{ axis: 'x' }`   slot: fresh div APPENDED to `#agent-panel-mount` after the chat mount (mount() replaces a slot's children) — `initAgentPanel()` turns `resize` into `--agent-panel-w` (clamped by `clampAgentPanelWidth`, `js/core/storage.js`), toggles `agent-panel-mount--resizing` (no transitions mid-drag) and stores the shown width on `resize-end` (`Storage.setAgentPanelWidth`, key `AGENT_PANEL_WIDTH`). App-lifetime.
...
- `MpiButton` (Add to history — destination toggle, MPI-887)   props: `{ icon:'layers', label: props.toHistoryLabel, size:'sm', variant:'ghost', toggleable:true, extraClasses:'mpi-media-picker__dest-btn' }`   slot: `#dest-slot` — rendered ONLY when the opener passes `toHistoryLabel`, and shaped like the FILTER button beside it (a form switch read as a settings row dropped into a control row). Its state rides out on both outcomes as `toHistory`; the picker never acts on it, because only the opener knows what a history entry would attach to. Not remembered between opens.
...
- `mountButton` (the media strip's `+` card, MPI-721)   props: `{ variant:'ghost', extraClasses:'mpi-prompt-box-media-strip__add' }`   slot: prepended into `.mpi-prompt-box-media-strip` — only when `props.stageMedia` is true, which ONLY `MpiGroupHistoryBlock` passes (`stageMedia: !isVideo`). Created ONCE and re-appended by `_renderStrip` (which wipes the strip's innerHTML), so it survives every repaint and keeps the strip from ever being `:empty`. Opens `MpiMediaPicker`, which portals to `<body>` and is destroyed on pick/import/cancel. **MPI-887:** the picker is handed `toHistoryLabel: 'Add to history'` here and nowhere else, so the second destination exists only where there IS an open card — a `toHistory` pick/import comes back as `stage-to-history` instead of a chip. **Desktop specs: this button does not exist without an installed prompt-capable model**, because the box itself does not mount — pin one (`pinOneModelInstalled`) or the spec is green locally and red on a weightless runner (`docs/testing-desktop-specs.md` trap 5).
- `MpiButton` (Agent/Prompt mode toggle, MPI-774)   props: `{ image:'assets/mascot/studio/logo.webp', info:'Switch to Agent mode (chat)', size:'sm', variant:'ghost', toggleable:true, active:_agentMode }`   slot: `#mode-toggle-slot` — sits directly after the text field, before enhance (bar order: neg · prompt · **mode** · enhance · model · cog · engine · run). Click writes `state.agentMode` (top-level replace, `_setAgentMode`) and toggles the `mpi-prompt-box--agent-mode` root class; `Events.onState('agentMode')` syncs the button (and re-applies the class) when another instance or component changes the key. While on, Enter in the textarea (no Shift) calls `_sendAgentTurn()` instead of running a generation — see `component-events-blocks.md` § MpiPromptBox `agent:send`. **Agent face (MPI-797, `_applyAgentView`):** the class hides every slot but `#textarea-slot` and `#mode-toggle-slot` and collapses the grid to `1fr auto` (CSS, not `.hide`); the textarea shows the agent's OWN text (`agentValue` via `_readMode`/`_writeMode`) with a usage placeholder; image chips are attachments (up to `AGENT_MAX_IMAGES`, no op up-jump or prune, badge = position, no frame pill); the `generation.run` hotkey sends the message and `generation.loop` does nothing; leaving the mode runs `_fitMediaToOperation()` (prune, up-jump, else trim the tail). A box mounted while the mode is on opens on this face.

---

## MpiAgentChat.js (internal mounts, MPI-774)

Mounted twice — see `shell.js` (panel, `standalone:false`) and `projectUI.js` (landing, `standalone:true`) above. `props.standalone` gates which of the two blocks below exist; the transcript and its per-entry bubbles are hand-built DOM, not components.

- `MpiInput` (textarea, standalone only)   props: `{ type:'textarea', placeholder:'Ask the agent…', autoHeight:true }`   slot: `#ac-input-slot`
- `MpiButton` (send, standalone only)   props: `{ icon:'generate', info:'Send (Enter)', size:'sm', variant:'primary' }`   slot: `#ac-send-slot`
- `MpiButton` ×2 (Yes / No, install-confirm card)   props: `{ text:'Yes, install'\|'No', variant:'primary'\|'secondary', size:'sm' }`   slot: fresh `document.createElement('div')` per card, appended inline into the transcript by `_appendConfirm()` — one card per `agent:confirm` SSE event (or a still-pending one replayed from `GET /agent/history`). Neither button is tracked in `_unsubs`/destroyed explicitly; the pair lives as long as the transcript row does (session-lifetime).

**Panel mode (`standalone:false`):** no input row of its own — the host `MpiPromptBox` collects text via its Agent/Prompt toggle and routes it in over the `agent:send` bus event; this instance's own `Events.on('agent:send', ...)` listener (guarded on `!props.standalone`) is the only consumer.

**Standalone mode (`standalone:true`):** drag-and-drop image files onto the root `el` are read via `FileReader.readAsDataURL` into `_pendingAttachments`, rendered as removable thumbs in `#ac-attachments`; Enter (no Shift) in the textarea sends by calling `agentSendMessage()` directly (no `agent:send` bus hop).
...
- `MpiButton` (**folder**, MPI-915)   props: `{ icon:'folder', variant:'ghost', size:'md', info:'Open the Third-party Flows folder' }`   slot: `ce('div')`, then `filterBar.el.appendTrailing(...)` — append, never mount; sits before Refresh. Click POSTs `/open-folder` with `userFlowsDir()` (the `dir` from the last `GET /user-flows`).
- `MpiButton` (**Refresh**, MPI-532)   props: `{ icon:'refresh', variant:'ghost', size:'md', info:'Refresh Flows from disk' }`   slot: `ce('div')`, then `filterBar.el.appendTrailing(...)`. Click → `loadUserFlows()`, reopen/close the drawer, `renderList()`, `reSyncInstalledModels()`; sets and clears its `loading` attribute.
