...
NOTE:    Ops LEFT the radial (MPI-356) — they live in the prompt box's op strip. There is no op→item mapping and no `refreshRadial`/`refreshGroupHistoryRadial` plumbing any more. The model PICKER also left (MPI-378) and has not come back — the prompt box's model button is still the only emitter of `ui:open-model-picker`. MPI-811's Models leg is a different thing: it emits `models:open` (the Model Library), not the picker.
