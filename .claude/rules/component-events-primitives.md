...
EMITS:   `select`    `{ action: string }`
         ...
         `open`      `{}`
         `close`     `{}`
LISTENS: Hotkeys 'tab' (`radialMenu.toggle`, MPI-811 — hold opens the PAGE context), Hotkeys 'control+tab' (`radialMenu.devToggle`, MPI-338 — hold swaps to the `dev` context, restored by `_hide()` on every close path; dev-mode-gated so inert in production), window keyup/mousemove (close on release — intentional exception for radial menu gesture)
NOTE:    **USER-FACING AGAIN as of MPI-811** — this reverses the MPI-378 note that stood here, which said DEV-ONLY and "do NOT re-add a Tab item". Hold Tab for the `root` context: four fixed destinations built by `navigation._userRadialItems()` and rebuilt on every `will-open` — Gallery, Models, Flows, Latest Workspace. `_syncRadial()` mounts in production now; only the `dev` context stays `APP_CONFIG.dev_mode`-gated. The MPI-378 Tab flipper (`workspace.flip`, `_flipWorkspace`) is gone: one key cannot be both a tap-flipper and a hold-menu.
NOTE:    Items may carry `angle` (degrees, 0 = right, -90 = top), which overrides the even ring spacing — MPI-811's four sit on the DIAGONALS, not the cardinal points. Icons must be FILL-based registry names: `_icon()` always fills, so a stroke-only entry (`gallery`, `seed`, anything `_stroke`/`ratio_`) renders as a solid blob.
NOTE:    🔴 **WHERE IT MOUNTS IS LOAD-BEARING, NOT COSMETIC.** `#radial-mount` is a direct child of `<body>` (`index.html`) because `MpiOverlay` stashes every sibling of its mount target into a `display: none` div — and a stashed radial still takes the keypress and still navigates on release while DRAWING NOTHING. Inside `.main-area` an open Flow stashed it; inside `#app-shell` any body overlay did. Out at body level only a body-mode overlay can reach it, and `MpiOverlay` § TRAP 1a spares it by name there (like `#titlebar` and the toast stack). It is `position: fixed` under the titlebar at `z-index: 19000` — above the overlay manager's `BASE_Z 10000 + 10/depth`, below the toast stack's 20000. **Do not move it back, and do not hide `#radial-mount` from any mode** — `focus-mode.css` did, and that was the same silent failure (MPI-811). If the radial must be inert somewhere, gate `radialMenu.toggle`'s `when`; hiding the element does not disarm the key.
NOTE:    The `when` gate's only surface test is the PAGE (gallery / group-history). Overlays do NOT block — the radial is the app's selector and has to reach out of an open Flow, the Flow Library and the Model Library alike (Fabio, 2026-09-19). A `.mpi-modal` DOES block: a modal is a question waiting on an answer, and navigating out from under one orphans it. Because the radial opens over things, every PAGE destination in `_syncRadial`'s `select` handler goes through `navigation._leaveOverlaySurfaces()` — `flow:suspend` FIRST (park, never close), then a bounded `Overlays.closeTopOverlay()` sweep so each overlay's own `hide()` restores what it stashed.
NOTE:    Dev actions (Components / Restart Engine) live on the `dev` context (Ctrl+Tab) — `navigation._syncRadial` sets it only in `APP_CONFIG.dev_mode`. **Flows LEFT that ring** in MPI-811: it is a user destination now. The old `extraItems`/`setExtraItems` API was removed (MPI-338).
...
NOTE:    Ops LEFT the radial (MPI-356) — they live in the prompt box's op strip. There is no op→item mapping and no `refreshRadial`/`refreshGroupHistoryRadial` plumbing any more. The model PICKER left with MPI-378 and CAME BACK with MPI-848: MPI-811's Models leg emits `ui:open-model-picker` (after `_leaveOverlaySurfaces()`), so the prompt box's model button is no longer the only emitter. Two emitters, ONE picker — the workspace Block still owns the instance and applies the choice. The leg falls back to `models:open` (the Model Library) only at zero installs, where the picker would open empty.
