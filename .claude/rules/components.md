...
- **One height per size** — `--control-h-{sm,md,lg}` (34/50/62px) via `min-height` on `.mpi-btn--sm|md|lg`, so a text and an icon button of one size can sit on a row. A non-button control (a boxed field / dropdown trigger, 39px) matches no button size — stretch the row to it. Full rule + the mount-host trap: `.claude/rules/dos_and_donts.md` § CSS & Styling rule 4.
