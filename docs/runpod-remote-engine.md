...
- **No volume USED-bytes from RunPod (MPI-169).** REST `/networkvolumes` returns only
  ...
  **Units (MPI-756 measured, MPI-763 unified).** Every storage GB the app prints is DECIMAL,
  like RunPod: the bar (`total` 60000000000 for a 60 GB volume, "54.5GB / 60GB"), the
  disk-full toast, the `free space` log lines and model sizes. Before MPI-763 the toast and
  logs printed BINARY GB (a 55 GB volume read "49.6 GB free of 51.2 GB"), so an older
  `app.log` reads ~7% low against a newer one. Dep `size` strings are still measured
  1024-based and parsed that way; only the display converts. RAM and VRAM stay binary.
...
**On-demand model auto-upload (MPI-82)** — live-verified 2026-06-17 (L4). A LoRA/upscale present locally but absent on the Pod is auto-uploaded at generate-time (`comfyController._uploadRemoteModels`) before `/prompt`. Wrapper endpoint: `POST /wrapper/models/upload` (image v0.4.9+). `forceLocal:true` skips upload. Presence check is a live `os.path.exists` on the volume every gen (no app cache). That check cannot see the IMAGE, so `remoteModelPresent` answers true for a `bakedOnPod` weight before asking (MPI-791) — the two baked upscalers read absent, and a user with no local copy got "Could not upload" instead of a generation. `/comfy/list-files` lists those weights while a Pod is connected, so a remote-only user sees them in the Upscale dropdown.
