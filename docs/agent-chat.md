...
| `read_memory` / `write_memory` | `{ file?, scope? }` / `{ file, title, text, hook?, scope? }` or `{ file, delete: true, scope? }` | `/connector/memory` for the OPEN project (`NO_PROJECT`); `scope: "global"` needs no project |
...
- **`GET /connector/memory[/:file]?folderPath=`** -> `{ ok, notes: [{ title, file, hook }] }` / `{ ok,
  ...
  created, warning? }` (`services/agentMemory.mjs`). `<project>/Agent/README.md` indexes one `<slug>.md` per note.
  **`delete: true`** in the POST forgets one (`{ ok, file, forgotten }`): its line goes, the file moves to
  `Agent/forgotten/` (Fabio 2026-09-26: the agent keeps its notes tidy; the user can still get one back).
  Cap 50 a list; from 40 a write carries `warning` (merge or delete stale ones). No DELETE route.
  Errors: `BAD_REQUEST` (400), `NOT_A_PROJECT`, `UNKNOWN_NOTE`, `NOTE_TOO_LONG` (4 KB), `MEMORY_FULL` (50).
