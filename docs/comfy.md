...
**Driver floor is checked at START, not install (MPI-902):** a driver whose CUDA major is below the installed torch's (`torch-*+cuNNN.dist-info`, read by `readEngineTorchCuda`) cannot run it at all — cu130 on a CUDA 12.6 driver dies with access violation 3221225477 and no word about drivers. `/comfy/start` refuses with `driverTooOldReason()` ("update to 580 or newer"), shown as the start-error dialog. Deliberately no fallback to the cu126 build: it cannot run Blackwell, is unsmoked, and would outlive the driver update. The header is `CUDA Version:` on old drivers and `CUDA UMD Version:` on r6xx — parse both.
