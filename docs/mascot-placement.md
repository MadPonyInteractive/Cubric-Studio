...
| Starting engine screen — **BUILT 2026-09-25** (MPI-906) | `MpiStartingComfy.js` `show()` mounts `mascotLoop`, `hide()` frees it | ComfyUI booting | Studio engine starting, looping | Studio |
| Model Library queued install — **BUILT 2026-09-25** (MPI-906) | `MpiTileSheet.js` `_setWaiting` (item `waiting` = the mascot key; the video exists only while queued), key from `MpiModelManager.js` `_waitingMascot` | install waiting its turn | working (rule 7) | the model's op |
| History peek — **BUILT 2026-09-25** (MPI-906) | `MpiGroupHistoryBlock.js` `_mascotShow` / `_mascotRelease` (src set only while shown) | waiting | working (rule 7) | op's |
