...
| Prompt box ledge, agent mode | `MpiPromptBox` top edge | always, while in agent mode | idle x3 / agent listening / agent thinking / agent answer ready | Studio |
...
| Agent chat box, 48px (the ledge may replace it, undecided) | `MpiAgentChat.js:44-94` (idle / waiting PNG) | idle ("Ask me anything") / user typing (no hook yet) / `_setWorking(true)` ("Thinking…") / reply lands | idle x3 / agent listening / agent thinking / agent answer ready, then back to idle | Studio |
