...
| Agent panel crew ledge — **BUILT 2026-09-22** (MPI-777 Phase 4) | `MpiAgentChat.js` `__crew` (panel only), a fixed 52px row on the composer's top rule, plus a 20px `studio/logo` in the header | rest / `_setWorking(true)`; guest on `generation:started` → `complete`/`cancelled`/`error` | Cosmo: the landing's `__ledge` at `--ledge-w: 115px` — `studio/peek` at rest, `studio/agent-thinking` while working, state line "listening" / "holding the thread". Guest: the NEWEST generation still running (any source — generations carry no agent tag), its `peek` looping, name + progress verb + elapsed timer, sliding in over 280ms. Every peek measured: the head spans columns ~95-515 of 620 in all five, so one framing fits all. Plays only while the panel is open away from the landing | Studio + op's (`getCommandAccent`) |
