...
- **Two stacked videos per member.** Assigning `src` to the visible one blanks it until the first frame decodes — five mascots blinking every few seconds — so the hidden one loads and starts and only then do they trade places. A `play()` promise from a swap that has been overtaken must not flip them, hence the sequence guard. **The trade is `handOverClip`, and it is the flicker fix (MPI-777, 2026-09-24):** the new clip shows on its first PRESENTED frame (`requestVideoFrameCallback`; `play()` resolves earlier), and the old one holds its last frame until the new one's NEXT frame, because a `<video>` going opacity 0 → 1 paints nothing for one frame. Flipping both at once blanked the mascot for a frame at ~half the swaps (proved on screencast frames). A 250ms bound drops the old clip in a window that is not drawing. The agent panel's crew (`MpiAgentChat.js`) imports the same function; any new mascot spot must too.
