...
- **Two stacked videos per member.** Assigning `src` to the visible one blanks it until the first frame decodes — five mascots blinking every few seconds — so the hidden one loads and starts and only then do they trade places. A `play()` promise from a swap that has been overtaken must not flip them, hence the sequence guard. **The trade is `handOverClip`, and it is the flicker fix:** the new clip shows on its first PRESENTED frame (`requestVideoFrameCallback`; `play()` resolves earlier), and both classes flip in that same frame. **The hidden clip sits at `opacity: 0.001`, never 0** — Chromium stops drawing a `<video>` at 0, so one going 0 → 1 paints nothing for a frame (a blank at ~half the swaps, MPI-777). Covering that by holding the old clip up a frame longer stacked two poses on every mid-clip swap such as a hover greet (MPI-908); 0.001 keeps the layer drawn and rounds to invisible. Both proved on screencast frames. The agent panel's crew (`MpiAgentChat.js`) and the generating card (`MpiGalleryGrid.js`) import the same function; any new mascot spot must too, with its hidden clip at 0.001.
