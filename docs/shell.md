...
- `back()`: Pops history.
- **Tab = the user radial (MPI-811).** `radialMenu.toggle` is bound by `MpiRadialMenu` itself, and the component is mounted by `_syncRadial()` on first entry into a workspace and destroyed in `handleNavigation(PAGE_LANDING)` — NOT app-lifetime, because `hotkeyManager` suppresses native Tab traversal the moment any `down:tab` handler exists, which would kill tabbing through the landing page's project form. Hold Tab, aim, release. Four fixed destinations on the DIAGONALS (Fabio's layout, and the reason `MpiRadialMenu` items may carry an `angle` that overrides the even ring spacing): **Gallery** top-left, **Projects** bottom-left, **Flows** top-right, **Latest Workspace** bottom-right. `_userRadialItems()` is rebuilt on every `will-open`, because the last leg dims (`disabled`, and the resolver refuses it) when `resolveFlipTarget(state.currentProject)` returns `null` — a project with no cards, an audio-only card (the gallery won't open those either), or a remembered card that was deleted. A project with exactly ONE card always resolves to it. The remembered card is `project.lastGroupId`, written by `_rememberGroup()` from the single choke point in `_loadView` (after the Block mounts, so restore-on-boot and any future entry path are covered) and cleared inside `projectService.removeGroup` — never at the four call sites. No `SCHEMA_VERSION` bump: the field is optional and absent means nothing to return to.
  **This replaced the MPI-378 → MPI-589 → MPI-611 Tab FLIPPER**, which is gone with `workspace.flip` and `_flipWorkspace()`. One key cannot be both a tap-flipper and a hold-menu — from key-down they are the same event — and the radial reaches every stop that ring had plus the landing page. MPI-611's parking rule survives intact: the Flows leg emits `flow:restore` first and only falls back to `flows:open` when nothing is parked, and the page legs go through `_leaveFlowSurface()`, which emits `flow:suspend` — hiding `MpiBaseFlow` WITHOUT its outward `close`, so the shell's MPI-345 destroy never fires and the same instance comes back with the same step, inputs and running job.
  🔴 **A BODY OVERLAY STASHES THE WHOLE `#app-shell`** into a `display: none` div (`MpiOverlay` § TRAP 1a), radial included — so a radial opened over one takes the keypress and navigates on release while drawing NOTHING. `radialMenu.toggle`'s when-gate therefore blocks on `.mpi-overlay--body` flat, Flow Library included: MPI-589's exception for it died with the tap-ring that needed it, and the Library has its own X and Escape. An **open flow** is the case that must still work, and it does: it mounts `main-area`, where `MpiOverlay` now SPARES `#radial-mount` from the stash the way it already spared `#shell-info-bar` and the toast stack, and `.mpi-radial` sits above the overlay at `calc(var(--main-overlay-z, 500) + 20)`. `tests/desktop/radial-menu.spec.js` covers the four destinations' angles, each leg, the dimmed one, the parked-flow round trip (it stamps the live node and asserts the SAME one comes back, so a re-mount cannot pass as a restore), the z war (`elementFromPoint` over an open flow — Playwright's own visibility check cannot see occlusion) and both Library blocks.
- **The radial mounts in PRODUCTION again (MPI-811).** `_syncRadial()` no longer early-returns on `!APP_CONFIG.dev_mode`; what stays dev-gated is the Ctrl+Tab `dev` context (Components / Restart Engine, MPI-338), and **Flows left that ring** now that it is a user destination. Models is still reached from the prompt box's model button, which emits `ui:open-model-picker`. Radial icons must be FILL-based names — `_icon()` fills its paths, so a stroke-only entry such as `gallery` renders as a solid blob (hence Gallery wears `grid`).
