...
**Images get a thumb too (MPI-319).** A 512px **WebP** at `.meta/<id>.thumb.webp` via `extractImageThumb` (same `ffmpegThumb.js`), written on save-generation + import in `routes/projects.js`. **`extractImageThumb` returns the path it ACTUALLY wrote** — callers pass a `.thumb.jpg` path and must use the return value, not the path they passed. An image past ffmpeg's compiled-in picture limit (`(w+128)*(h+128) > INT_MAX/8`, so a 16K photo) thumbs through sharp instead, upright, same widths and WebP (MPI-926); everything ffmpeg can decode, BMP included, stays on ffmpeg. An imported SVG never reaches either: `/project-media/:id/upload` renders it to PNG (its declared size, long edge raised to 2048, capped at 4096) and does not keep the `.svg`, because every tool here reads pixels (MPI-933). `placeContentAsset`, the reference store an agent stages into and a Flow input drop lands in, does the same (MPI-934). The viewer still opens full-res (`filePath`). Decoding 100+ full-res 4K PNGs was the scroll-jank cause (~179x heavier than thumbs). Pre-existing images (made before thumbs) backfill via `POST /backfill-media-derivatives`, fired fire-and-forget on project load (`projectService._backfillMediaDerivatives`), which patches live items + rebuilds the grid. Derivatives live/die with the sidecar — the GC and the delete paths match `DERIVATIVE_RE` (`<id>.thumb.*` / `<id>.proxy.*`) by prefix rather than by an extension list, because three separate lists had to be edited in lock-step and a missed one leaks a file per asset.

**"Cleanup assets…" deletes them on purpose, and that is what makes it the PRE-SHARE step (MPI-821).** `POST /project/cleanup-assets` (`cleanupRebuildableAssets`) drops every `<id>.thumb.*` and `<id>.proxy.*` — a video proxy is the bulk of a project that is not a master file — and they come back on the next project load. Two traps it has to respect, both of which read as working code if you get them wrong:
- **It must NULL `thumbPath` / `thumbPathLg` / `proxyPath` in the sidecar with the files.** `/backfill-media-derivatives` gates on the SIDECAR, never on disk (`if (meta.thumbPath) continue`), so leaving them set means the rebuild skips the item forever. An image card falls back to `filePath`; a **video card cannot**, so it is a blank grey tile for the life of the project.
- **It must NOT use `DERIVATIVE_RE`**, which also matches `<id>.splat.ply`. A splat is a MASTER — the still is rendered FROM it (MPI-623) — so cleanup carries its own narrower `CLEANUP_DERIVATIVE_RE` (`thumb|proxy`). Test: `tests/cleanup-derivatives.test.cjs`.
...
Drive seed from desired items-per-row, not pixel: `target = ((containerWidth - (N-1)*gap) / (N * aspectRef)) * 0.92`. `aspectRef` 1.6. Justified-layout per-row rescaling collapses any two seed pairs that land in the same items-per-row band → two adjacent pixel targets produce identical visual output. Current map: `ITEMS_PER_ROW_TARGET { 1:6, 2:4, 3:3, 4:2 }`. Recompute on BOTH a `state.gallerySizeLevel` change (toolbar slider or `+`/`-`) AND ResizeObserver.

## Archive is a SCOPE, not a filter (MPI-678)

`state.gallerySort.scope` (`'active'|'archived'`) is **subtractive**: an archived group is absent from the active gallery and vice versa. It gates FIRST inside `matchesGallerySort` (`js/utils/galleryFilter.js`), ahead of the kinds and flags, which is what keeps the FILTER panel working inside the archive — the one bucket big enough to need it; `tests/desktop/gallery-archive.spec.js` asserts Images-inside-the-archive for exactly this reason. It is also why Archive stays its own toolbar toggle rather than a panel row. Kinds, flags, the panel and the toolbar: [gallery-filters.md](gallery-filters.md).

`gallerySort` is deliberately **NOT** mirrored to `Storage` — in-memory, so it resets to `active` and unfiltered every launch; nobody should relaunch into a gallery that looks wiped. The archive has an empty state (`.mpi-gallery-grid__scope-empty`) because a blank archive is the single way this reads as deletion; the active gallery gets one only when a filter hides every card.
...
`MpiGalleryGrid` card chrome uses inverse `galleryShowInfo` model: info OFF = clean media until hover reveals metadata/actions; info ON = metadata by default, hover hides metadata and shows actions. State/preview/selection badges and the kind chip ([gallery-filters.md](gallery-filters.md)) stay persistent. The bottom-right kind chip is on EVERY card that has an item (MPI-736 round 7 — a card with no selected item shows none, or the catch-all `image` row would make it claim to be one), and its glyph wears the media type's family accent via `[data-accent]` on the chip itself, not an ink. Local chip/button backgrounds, not card-wide radial scrims. Prompt excerpts stay out of gallery cards; bottom metadata = compact dimensions/time only.

**Top-right action column (MPI-564).** One flex column, fixed order: Mark → Notes → Reuse. The mark (MPI-785, [gallery-filters.md](gallery-filters.md) § Card marks) is hover-revealed but stays visible while the card is marked; Reuse is hover-only and hides when the selected item has no reusable prompt; the **Notes marker is persistent** — `notesWrap.style.display` is driven by `selected?.notes?.trim()`, so it appears only on an annotated card and needs no hover, which is the point (scanning a shared project for annotated cards). Clicking it emits the same `card-notes` event as the context-menu entry. Notes live on the **selected history item** (`item.notes`, sidecar-persisted), not the group — so the marker follows `selectedIndex`. A card's `_getGroupRenderKey` carries a `notes` flag; without it a notes edit that arrives via a full grid re-render (rather than `refreshGroup`) would not repaint the marker.
...
**A third consumer rides it whether you want it or not: `dataTransfer.files` (MPI-884).** Chromium synthesises a `File` from the dragged `<img>`'s OWN image resource, and a card's `<img>` is the 512 `.thumb.webp` rendition `pickImageRendition` picks until the card promotes. So an in-app drop target that reads `dataTransfer.files` silently receives the THUMBNAIL'S BYTES — not the card, not a resize of it, the rendition file itself. Proven 2026-09-22: the agent chat's staged attachment was byte-identical (sha256) to `Media/.meta/<itemId>.thumb.webp`, 512x682 for a 768x1024 card, and had already been placed into `.preview-assets/` by a generation. **Any in-app drop target reads `application/mpi-media` FIRST** — it is the only thing in the drag that names the card's real file. `MpiPromptBox._handleMediaDrop` and `MpiAgentChat`'s drop (via `cardReference` in `js/utils/mediaActions.js`) both do; `files` is the OS-file-drop path behind it. The agent chat never falls through: a card drop is the card or nothing (MPI-867). A drop target that handles a card must also `stopPropagation()`, because `MpiPromptBox` takes every card drop that reaches `window`.

## Hover audio + scroll-stop

MPI-132: hovering a gallery VIDEO card unmutes+plays its `<video>`; hovering an AUDIO card plays its hidden `<audio>`. Gated by the VOLUME slider in `MpiGalleryToolbar` — `state.galleryVolume`, Storage-mirrored (`mpi_gallery_volume`, default 0.8), followed by a setup-scope `_volume`. **0 IS the mute**, and the two card kinds treat it differently on purpose: an audio card skips hover play entirely (silent playback would only show a lying stop icon), a video card still previews but stays muted. The old "Play audio on hover" Settings toggle and its `mpi_play_audio_on_hover` key were REMOVED when the slider landed — the slider is the only control. One-card-at-a-time via `_stopOtherGalleryMedia(except)` covering BOTH `audio[data-src]` AND `video.mpi-group-card__thumb--video`. SCROLL BUG: `mouseleave` does NOT fire when the card scrolls out from under a STATIONARY cursor. Do NOT rely on mouseleave alone for "stopped hovering" in a scrollable list.
