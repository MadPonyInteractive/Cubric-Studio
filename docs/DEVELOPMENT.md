# Developing Cubric Studio

This guide is for developers who want to run Cubric Studio from source or work
...
- **A slow canvas, video or UI on a user's box: grep `\[gpu\]` first** (MPI-794). One line per boot says whether Chromium draws on the GPU (`canvas=enabled`) or in software (`unavailable_software`), and names the adapter in use (`renderer="ANGLE (NVIDIA, …)"` vs the integrated one or `Microsoft Basic Render Driver`). A `GPU process gone` warn plus a second status line means it crashed mid-session and may have come back in software. It is a boot line, so the rule above applies: grep the whole `logs/` directory.
...
**A dead stdout is not a boot crash, and used to look exactly like one (MPI-514).** Whenever the thing reading main's stdout goes away — an agent piping `npm run app:isolated` into `head -1`, a killed launcher, a closed terminal — the console mirror in `routes/logger._write` fails ASYNCHRONOUSLY: the call returns, the stream emits `'error'` afterwards, so that function's own try/catch (written for a synchronous EIO) never sees it. Node then re-raised it through `uncaughtException` and a modal "Cubric Studio failed to start" box appeared, quoting an EPIPE stack, on a healthy app. `main.js` now attaches `'error'` handlers to `process.stdout`/`stderr` that swallow `EPIPE`/`EIO` only; every other stdio error still goes to `reportFatal`. The file log is the durable sink, so the dropped mirror costs nothing. Note the electron npm CLI wrapper has the same EPIPE fragility and is third-party — piping a launch into `head`/`grep -m1` still kills the app, just quietly now.

Still invisible: anything that kills the process before those handlers register (a require-time throw in `main.js`'s first nine lines), and anything that prevents Electron from starting at all. For those, ask the user to run the app from a terminal — `CubricStudio.exe` from a console on Windows (MPI-387 deleted the `.bat`/`.vbs` launchers), `start-with-terminal.sh` on Linux — because main-process stdout never reaches any log file.
