...
- **A stop wins a start's prelude (MPI-764).** `download()` awaits dir/stat/marker work before it starts NDH; `cancel()`/`stopKeep()` set a stop flag it checks before starting, and await that in-flight prelude before stopping and deleting. Before this, a cancel (or the ENOSPC `cancelAllDownloads`) inside the window let the stream start anyway, and rewrote the marker `cancel()` had just deleted. `cancelAllDownloads()` returns a promise that settles when every `stopKeep()` has.
