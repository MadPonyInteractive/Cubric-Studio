...
**Reuse prompt recall (`state.promptReuseOptions` / `state.promptReuseSource`):** Ask is a behavior flag; Gallery can reuse Original or Current. I2V reads frame snapshots from the sidecar, `snapshots[].filePath || snapshots[].url` (`_mediaItemsFromPreviewAssets`) — the old per-item `<itemId>/startFrame.png` disk probe was removed in MPI-227, so the sidecar ref is authoritative. **MPI-821 made that ref the SOURCE CARD's own url**: a gallery-sourced input is no longer copied, so a sidecar written since carries `url` and no `filePath`. `filePath` survives only on sidecars written while the copy store was live. Gallery Reuse source `current` uses the card's active `selectedIndex` entry. MPI-127: `_mergeReuseMedia(frames, saved)` in `js/utils/promptReuse.js` — preview-asset frames are authoritative for IMAGES, saved media supplies every OTHER type (audio, non-frame video); if frames present, saved images dropped to avoid dup start-frame chip. MPI-227: the Reuse dialog gains **Use Video** + **Use Audio** toggles (beside Use Images), each op-gated per source (`payloadHasReusableVideos`/`payloadHasReusableAudio`); saved video/audio is op-gated in `buildPromptReusePayload` (extend/interpolate/videoUpscale/resizeVideo accept video; LTX t2v_ms/i2v_ms accept audio). **A reuse resolves against the SOURCE CARD now (MPI-821)**, so deleting that card loses its inputs — Archive is how a user keeps one. `resolvePromptReuseMediaItems` HEADs every url first, so the failure is a `ui:warning` toast (comfyController tags the 404 `code:'input_asset_deleted'`), never the error dialog or a broken chip. The `.preview-assets` store itself is still permanent and still only emptied by the manual projects-page "Cleanup assets…" command (`POST /project/cleanup-assets`), but it now holds only files with **no gallery card** — agent `placeAsset`, Flow OS-file drops.
