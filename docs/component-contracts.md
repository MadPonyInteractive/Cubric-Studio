...
- **One definition of a row.** `js/components/loraSlotParts.js` exports `buildStrengthsRow` / `buildBypassBtn` / `applyBypass` plus the path helpers, and both surfaces build from it with their own `bem` prefix. The bounds (`-2..2`, step `0.05`, 2 decimals) and the bypass contract (`aria-pressed` is the only source of truth, MPI-588) have to agree byte-for-byte — the two views write the same key, so a drift between them shows up as a number that changes when the user looks away.
...
MpiToast caps visible toasts at `MAX_VISIBLE_TOASTS = 2`. Visible count = live DOM query (`qsa(':scope > .mpi-toast:not(.mpi-toast--queued)', stack)`), NEVER a counter var. Queued toasts mount INSIDE `.mpi-toast-stack` hidden via `.mpi-toast--queued { display:none }` — NEVER park a toast in `document.body`. Queued toasts get NO timer until promoted. `dismiss()` is idempotent. One clean drain path. **Clicking anywhere on a toast calls that same `dismiss()`** (MPI-784) — persistent (`duration: 0`) toasts included, so a click frees the slot and hands the countdown on exactly like a timeout; pinned by `tests/desktop/toast-click-dismiss.spec.js`. Verify any toast change with a burst test: fire 5+ toasts, assert never >2 visible, none at top-left/out-of-stack, full drain to zero.
...
**(4) Playback follows a CHOSEN output device, applied from one place (MPI-803).** The app played to whatever endpoint the OS called default, which on a box with a virtual mixer (SteelSeries Sonar and friends) is routinely not the one the user listens to — the app then emits correctly into silence: real Windows session, real peak, nothing audible, and no UI anywhere that says which endpoint it reached. Settings -> Audio -> Output is the LAST plate in that section (input first) and stores a deviceId (`Storage.getAudioOutputDevice`, `mpi_audio_output_device`, `''` = OS default); its **Test** button plays the notification chime through `applySink`, which is why that function returns a boolean — a wrong device is inaudible, so a test that swallowed the failure could not be told apart from one not yet pressed. Ordinary playback still swallows it. A pinned device also OVERRIDES the user's own per-app OS routing, which is why nothing auto-picks one. `js/utils/audioOutput.js` applies it through **one capture-phase `play` listener on `document`** — `play` does not bubble, but a capture listener is still on the path to any in-document media element, so all ~8 playback sites are covered without a `setSinkId` call in each. An element that never enters the DOM is the one gap, so MpiToast's detached chime calls `applySink` itself. A rejected `setSinkId` is swallowed to a warn-once: the fallback is the wrong device, never a dead playback. `setSinkId` is spec'd behind a `speaker-selection` permission, but MEASURED on this Electron (2026-09-17) the `media` grant already covers it — a real output deviceId is accepted with `speaker-selection` absent from `ALLOWED_PERMISSIONS`, so it is deliberately not in that set and the spec above asserts the day that changes.
...
- **The filter is the gallery's own (MPI-785).** The same FILTER button and panel (`js/components/galleryFilterPanel.js`) and the same `matchesGallerySort` contract, run on a LOCAL sort object so narrowing a slot never refilters the gallery. A card is kinded by its SELECTED item, as the gallery does, so a `type: 'video'` group whose selected take is an image lists under Images in both (the spec's fixture is exactly that card). The sort opens on the gallery's `order`, with every listed kind whose `type` (`ASSET_KINDS`) differs from the slot's hidden. Tiles show the card's mark, read-only. The panel lifts its z-index above the picker's modal (MpiPopup's own 9999 sits under it).
