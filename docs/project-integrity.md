...
- **Nothing in the renderer calls `/remove-project-path` — there is no unregister UI.** The only pruning a user can reach is the `delete-project` side effect above. Unregistering a parent without deleting its projects means calling the route by hand (DevTools console).
- **A registry entry does not stay removed.** The self-heal migration above is unconditional: `list-projects` writes *every* caller-supplied `extraPaths` entry into the durable registry, and that registry is one shared file in `<Documents>` that every app instance and connector client writes to. So removing an entry only sticks once no client anywhere still holds it in its localStorage mirror — clear `mpi_extra_project_paths` in each, or the next Landing load re-adds it. Symptom seen in the wild: a bare `%TEMP%` entry (agent scratch projects live directly under it) that returns minutes after removal and makes every scan walk the whole temp tree.
