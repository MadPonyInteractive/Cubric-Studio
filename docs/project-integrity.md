...
- **Nothing in the renderer calls `/remove-project-path` — there is no unregister UI.** The only pruning a user can reach is the `delete-project` side effect above, so a parent that holds no project *directly* (a bare `%TEMP%`, say) has nothing to delete and cannot be pruned from the UI at all. Unregistering by hand means calling the route (DevTools console). Note this also makes the delete dialog's "Keep it" branch (uncheck *Also delete files from disk*) a no-op on the list: the folder stays, so it is its own surviving sibling and nothing is pruned.
