...
| **MCP server for OUTSIDE agents** (`POST /mcp`: Claude Code, Claude Desktop's `.mcpb` bridge, Codex, Gemini; the 17 tools, 45 s waits, spend gate, cancel; why Codex never sees MCP tools without a skill; cold-test fencing) | [mcp-server.md](mcp-server.md) |
