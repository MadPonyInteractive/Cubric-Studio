...
| **In-app agent chat** (the Agent \| Prompt toggle's server-side loop: one conversation per project, tool schemas, `/connector/*` + `/agent/*` routes, SSE events, endpoint profiles and keys, `look` coordinates) | [agent-chat.md](agent-chat.md) |
