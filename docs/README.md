...
| **WRITING a desktop spec** — the specs that exist + the six traps they paid for (flow overlays, the frame with no geometry, the stubbed-global-state race, PROMPT mode's missing canvas) | [testing-desktop-specs.md](testing-desktop-specs.md) |
| **Master is RED / a push is blocked / agents keep pushing `--no-verify`** — five-command diagnosis, the four things that generate reds, the done gate vs the master gate | [red-master.md](red-master.md) |
...
| **Flow packages** (third-party Flows as data-only folders in `user_flows/`: manifest format, allowed ids, the title law the validator enforces, the no-markup rule, drop install / Refresh, the linter) | [flow-packages.md](flow-packages.md) |
| **In-app agent chat** (the Agent panel's server-side loop: one conversation per project, tool schemas, `/connector/*` + `/agent/*` routes, SSE events, endpoint profiles and keys, `look` coordinates) | [agent-chat.md](agent-chat.md) |
| **MCP server for OUTSIDE agents** (`POST /mcp`: Claude Code, Claude Desktop's `.mcpb` bridge, Codex, Antigravity; the 17 tools, 45 s waits, spend gate, cancel; why Codex never sees MCP tools without a skill; cold-test fencing) | [mcp-server.md](mcp-server.md) |
| **Why the in-app agent did the wrong thing** (what it was SEEN to do live and the root cause: facts that never reached it, names that sound like values, negation in prompts, i2i over edit, unrecorded `look`s, the step cap; what worked; what Fabio refused or deferred) | [agent-findings.md](agent-findings.md) |
